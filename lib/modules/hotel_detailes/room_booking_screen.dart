...
class _RoomBookingScreenState extends State<RoomBookingScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          getAppBarUI(title: widget.hotelName),
          Expanded(
            child: context.read<GeneralProvider>().listRoomsModel != null
                ? ListView.builder(
                    padding: const EdgeInsets.all(0.0),
                    itemCount: romeList.length,
                    itemBuilder: (context, index) {
                      var listRooms =
                          context.read<GeneralProvider>().listRoomsModel;
...
class getAppBarUI extends StatelessWidget {
  getAppBarUI({Key? key, this.title}) : super(key: key);
  String? title;
